...
The analysis pipeline is distributed in two equivalent formats — choose whichever fits your workflow. Both files implement the same logic and produce the same results.

#### Option A — Jupyter / Colab notebook *(recommended)*

`Analysis_Pipeline_Cleaned.ipynb` preserves the original Colab authoring environment and is rendered natively by GitHub, so reviewers and readers can inspect each step without having to clone the repository.

```bash
jupyter notebook python_pipeline/Analysis_Pipeline_Cleaned.ipynb
```

To run it in Google Colab, open the notebook on GitHub and click **Open in Colab**, or upload the `.ipynb` file directly to [colab.research.google.com](https://colab.research.google.com).

#### Option B — Python script

`analysis_pipeline.py` exposes the same pipeline as a command-line script for local execution or integration into other Python workflows.
...
Both files are organized into four sequential sections:
